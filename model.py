...
class CharEmbedding(nn.Module):
    ...
    """
    def __init__(self, char_size, char_dim):
        super(CharEmbedding, self).__init__()
        self.charembed = nn.Embedding(char_size, char_dim)
...
class BIDAF_Model(nn.Module):
    def __init__(self, char_size, vocab_size, char_dim = 100, word_dim=100, dropout=0.2):
        super(BIDAF_Model, self).__init__()
        self.d = char_dim + word_dim
        self.char_embedding = CharEmbedding(char_size= char_size, char_dim=char_dim)
        self.word_embedding = nn.Embedding(num_embeddings=vocab_size,
                                           embedding_dim=word_dim)
        self.highway_layer =  Highway_Network(self.d,self.d)
        ...
        self.lstm_U = nn.LSTM(input_size=self.d, hidden_size=self.d, num_layers=1,
                              batch_first=True, bidirectional=True)
        self.lstm_M = nn.LSTM(input_size=8*self.d, hidden_size=self.d, num_layers=1,
                              batch_first=True, bidirectional=True)
